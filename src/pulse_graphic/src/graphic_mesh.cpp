...
void pulse_graphic_mesh_update_vertices(pulse_app_t app, pulse_mesh_t* mesh, const void* data, uint32_t count) {
    ...
    if (st && mesh) {
        pulse_graphic_mesh_ref ref{};
        if (pulse_graphic_mesh_acquire(app, *mesh, &ref)) {
            auto* m = ref.ptr;
            ...
            pulse_graphic_mesh_release(app, &ref);
        }
    }
}
...
bool pulse_graphic_mesh_acquire(pulse_app_t app, pulse_mesh_t handle, pulse_graphic_mesh_ref* mesh_ref) {
    pulse_asset_ref ref{};
    if (pulse_asset_acquire(app, pulse_graphic_mesh_to_handle(handle), &ref)) {
...
void pulse_graphic_mesh_release(pulse_app_t app, pulse_graphic_mesh_ref* mesh_ref) {
    pulse_asset_ref ref{ pulse_graphic_mesh_to_handle(mesh_ref->handle), nullptr };
