...
namespace HGEGraphics
{
	...
	{
		CompiledResourceNode(const char* name, ManageType type, uint16_t width, uint16_t height, uint16_t depth, ECGPUTextureFormat format, PulseTextureData* texture, uint8_t mipCount, uint8_t arraySize, uint32_t parent, uint8_t mipLevel, uint8_t arraySlice);
		CompiledResourceNode(const char* name, ManageType type, uint32_t size, PulseGraphicsBufferData* imported_buffer, ECGPUResourceTypeFlags bufferType, ECGPUMemoryUsage memoryUsage);
		...
		PulseTextureData* imported_texture;
		const uint16_t width;
		const uint16_t height;
		const uint16_t depth;
		const ECGPUTextureFormat format;
		PulseGraphicsBufferData* imported_buffer;
