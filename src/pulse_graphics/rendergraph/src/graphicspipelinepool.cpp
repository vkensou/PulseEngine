...
namespace HGEGraphics
{
	...
	GraphicsPipeline* GraphicsPipelinePool::getGraphicsPipeline(RenderPassEncoder* encoder, PulseShaderData* shader, PulseMeshData* mesh)
    {
		return getGraphicsPipeline(encoder, shader, mesh->prim_topology, mesh->vertex_layout);
	}

	GraphicsPipeline* GraphicsPipelinePool::getGraphicsPipeline(RenderPassEncoder* encoder, PulseShaderData* shader, ECGPUPrimitiveTopology prim_topology, const CGPUVertexLayout& vertex_layout)
	{
		auto key = PSOKey
		{
			.shader = shader,
