...
GpuBlockRef alloc_ubo_block(pulse_renderer_state& state, RendererView& view, uint32_t size) {
    ...
    block.cpu_data = state.write_packet().ubo_cache.acquire(aligned_size, state.frame_index, block.size);
    block.used = aligned_size;
...
uint32_t alloc_feature_ubo_column(FeaturePrepareContext& ctx, DrawItem& item, uint32_t set, uint32_t binding, uint32_t size) {
    ...
    col.set = set;
    col.binding = binding;
    col.block_ref = alloc_ubo_block(*ctx.state, view, size);
    view.ubo_columns.push_back(col);

    const uint32_t index = (uint32_t)view.ubo_columns.size() - 1;
    if (item.feature_column_count == 0) item.feature_column_start = index;
    ++item.feature_column_count;
    return index;
}

static void bind_ubo_column(PulseRenderPassEncoder* encoder, const RendererView& view, const RendererUboColumn& col) {
