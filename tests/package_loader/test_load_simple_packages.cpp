...
int main() {
    ...
    assert(pulse_package_loader_load_packages(loader, 1, search_paths, 2, entries) == PULSE_PACKAGE_LOAD_RESULT_OK);
    ...
    assert(pulse_package_loader_load_packages(loader, 1, search_paths, 1, &dup) == PULSE_PACKAGE_LOAD_RESULT_ERROR_DUPLICATE_PACKAGE);
