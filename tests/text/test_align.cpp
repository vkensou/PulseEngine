...
static PulseTextBlockDesc align_desc(uint32_t chain, EPulseTextAlignH align_h, EPulseTextAlignV align_v, float line_height) {
    PulseTextBlockDesc desc = text_desc(chain, kSize, line_height);
...
int main() {
    ...
    const PulseTextBlockDesc desc_center = align_desc(chain, PULSE_TEXT_ALIGN_H_CENTER, PULSE_TEXT_ALIGN_V_TOP, 0.0f);
    PulseTextLayout* centered = pulse_text_layout(app, &desc_center, "ab cd", box_w, 0.0f);
    ...
    const PulseTextBlockDesc desc_right = align_desc(chain, PULSE_TEXT_ALIGN_H_RIGHT, PULSE_TEXT_ALIGN_V_TOP, 0.0f);
    PulseTextLayout* righted = pulse_text_layout(app, &desc_right, "ab cd", box_w, 0.0f);
    ...
    PulseTextLayout* natural_centered = pulse_text_layout(app, &desc_center, "ab\ncd", 0.0f, 0.0f);
    ...
    const PulseTextBlockDesc desc_middle = align_desc(chain, PULSE_TEXT_ALIGN_H_LEFT, PULSE_TEXT_ALIGN_V_MIDDLE, 0.0f);
    PulseTextLayout* middled = pulse_text_layout(app, &desc_middle, "ab", box_w, la * 4.0f);
    assert(fabsf(middled->p_instances[0].y - ((la * 4.0f - la) * 0.5f + ascent + glyph_a.y0)) < 1e-3f);
    pulse_text_layout_free(app, middled);

    const PulseTextBlockDesc desc_top = align_desc(chain, PULSE_TEXT_ALIGN_H_LEFT, PULSE_TEXT_ALIGN_V_TOP, 1.5f);
    const PulseTextMeasure scaled = pulse_text_measure(app, &desc_top, "a\nb", 0.0f);
