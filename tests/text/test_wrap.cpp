...
int main() {
    ...
    const PulseTextMeasure natural = pulse_text_measure(app, &desc, "Hello world", 0.0f);
    ...
    const PulseTextMeasure wrapped = pulse_text_measure(app, &desc, "Hello world", box_w);
    ...
    const PulseTextMeasure overlong = pulse_text_measure(app, &desc, "aaaaaaaaaa", aaa_w);
    assert(overlong.line_count == 4);
    assert(fabsf(overlong.width - aaa_w) < 1e-3f);

    const float cjk3 = run_width(app, chain, "中中中", kSize);
    const PulseTextMeasure cjk_wrap = pulse_text_measure(app, &desc, "中中中中中中", cjk3 * 1.1f);
    assert(cjk_wrap.line_count == 2);
    assert(fabsf(cjk_wrap.width - cjk3) < 1e-3f);
    const PulseTextLayout* cjk_layout = pulse_text_layout(app, &desc, "中中中中中中", cjk3 * 1.1f, 0.0f);
    ...
    const PulseTextMeasure explicit_break = pulse_text_measure(app, &desc, "ab\ncd", 0.0f);
    ...
    const PulseTextMeasure crlf = pulse_text_measure(app, &desc, "a\r\nb", 0.0f);
    assert(crlf.line_count == 2);
    const PulseTextMeasure trailing_newline = pulse_text_measure(app, &desc, "ab\n", 0.0f);
    assert(trailing_newline.line_count == 2);
    const PulseTextMeasure empty = pulse_text_measure(app, &desc, "", 100.0f);
    assert(empty.line_count == 0 && empty.width == 0.0f && empty.height == 0.0f);

    const float ab_w = run_width(app, chain, "ab", kSize);
    const PulseTextLayout* trimmed = pulse_text_layout(app, &desc, "ab ab", run_width(app, chain, "ab ", kSize), 0.0f);
    ...
    const PulseTextMeasure kerned = pulse_text_measure(app, &desc, "AV", 0.0f);
    assert(fabsf(kerned.width - run_width(app, chain, "AV", kSize)) < 1e-3f);

    const PulseTextMeasure fallback = pulse_text_measure(app, &desc, "中A", 0.0f);
    assert(fabsf(fallback.width - run_width(app, chain, "中A", kSize)) < 1e-3f);
    const PulseTextLayout* emoji_layout = pulse_text_layout(app, &desc, "\xF0\x9F\x98\x80!", 0.0f, 0.0f);
