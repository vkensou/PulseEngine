...
- 模块名优先取自 packageinfo/package.json 的 `name` 字段（与包名、插件名保持一致）；
  没有 packageinfo 或缺少 `name` 时，回退用头文件名（去掉 `.h`）。
- 例如模块名 `snake` 会生成 `pulse_add_snake_plugin`、`snake_plugin_build` /
  `snake_plugin_shutdown`，插件名与模块名一致（`kPluginName = "snake"`），
  以及 `pulse_package_register` 导出入口。
...
- 可通过可选参数 `[packageinfo.json]`（例如 `examples/snake/package.json`）
  传入 manifest，生成器会读取其中的 `name` 与 `dependencies`。
